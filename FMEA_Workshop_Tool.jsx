...
/* =========================================================================
   ...
   Data disimpan lewat window.storage (shared) sehingga semua peserta yang
   membuka artifact ini melihat data sesi yang sama secara real-time (polling).
   ========================================================================= */
...
function roundRating(avg, max) {
  ...
  const r = Math.round(avg); // 4.4 -> 4 ; 4.5 -> 5 (round-half-up, sesuai aturan)
...
// ---------------------------------------------------------------------------
// UTIL — kode sesi, storage helpers, import excel
// ---------------------------------------------------------------------------
...
const sKey = (code) => `fmea_session:${code}`;
const pPrefix = (code) => `fmea_participant:${code}:`;
const pKey = (code, id) => `fmea_participant:${code}:${id}`;
...
async function storListVals(prefix) {
  try {
    const idx = await window.storage.list(prefix, true);
    if (!idx || !idx.keys || !idx.keys.length) return [];
    const out = [];
...
function mapRowToFM(row, idx) {
  ...
  const title = findVal(row, ['potential', 'failure']);
  return {
    no,
...
function FacilitatorSetup({ onBack, onSessionReady }) {
  ...
  async function createSession() {
    ...
    const session = {
      code, name: name.trim(), facilitator: facilitator.trim(),
      createdAt: Date.now(), fmList: [], fmStatus: {}, status: 'active',
    };
    await storSet(sKey(code), session);
    setBusy(false);
  ...
  async function loadSession() {
    ...
    if (!code) return;
    setBusy(true);
    const s = await storGet(sKey(code));
    setBusy(false);
    if (!s) { setErr('Kode sesi tidak ditemukan.'); return; }
    onSessionReady(s);
  }
...
function ImportTab({ session, onUpdateSession }) {
  ...
  async function confirmImport() {
    const fmStatus = { ...session.fmStatus };
    preview.forEach((fm) => { if (!(fm.no in fmStatus)) fmStatus[fm.no] = 'locked'; });
    const updated = { ...session, fmList: preview, fmStatus };
    await storSet(sKey(session.code), updated);
      ...
      {preview && (
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          ...
          <div className="flex justify-end gap-2 mt-3">
            ...
            <button onClick={confirmImport} className="px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg">
              Gunakan {preview.length} Failure Mode Ini
            </button>
          </div>
        </div>
      )}
...
function ControlTab({ session, onUpdateSession }) {
  async function setStatus(fmNo, status) {
    ...
    const updated = { ...session, fmStatus };
    await storSet(sKey(session.code), updated);
    onUpdateSession(updated);
  }
...
function FacilitatorDashboard({ session, onUpdateSession, onExit }) {
  ...
  const refresh = useCallback(async () => {
    const s = await storGet(sKey(session.code));
    if (s) onUpdateSession(s);
    const ps = await storListVals(pPrefix(session.code));
    setParticipants(ps);
...
function ParticipantJoin({ onBack, onJoined }) {
  ...
  async function join() {
    ...
    setBusy(false);
    if (!session) { setErr('Kode sesi tidak ditemukan. Cek kembali dengan fasilitator.'); return; }
...
function ParticipantMain({ initialSession, participant, onExit }) {
  ...
  async function submitVote(fm, vote) {
    const updatedVotes = { ...myVotes, [fm.no]: vote };
    setMyVotes(updatedVotes);
    const updatedParticipant = { ...participant, votes: updatedVotes };
    await storSet(pKey(session.code, participant.id), updatedParticipant);
    setVotingFM(null);
    refresh();
  }
    ...
    <div className="max-w-3xl mx-auto p-5">
      ...
      {votingFM && (
        <VotingForm fm={votingFM} existingVote={myVotes[votingFM.no]} onCancel={() => setVotingFM(null)}
          onSubmit={(vote) => submitVote(votingFM, vote)} />
      )}
    </div>
  );
}
